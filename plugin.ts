    ...
    app.services.waitForItemByType(ScriptingService, (scriptingService: ScriptingService) => {
      scriptingService.registerFunction("getSocialAccount", {
        ...
        async execute(this: ScriptingThis): Promise<string> {
          return JSON.stringify(await this.agent.requireServiceByType(SocialMediaService).getCurrentAccount(this.agent));
        },
      });
      ...
      scriptingService.registerFunction("getRecentSocialPosts", {
        ...
        async execute(this: ScriptingThis, limit?: string): Promise<string> {
          ...
          const posts = await this.agent.requireServiceByType(SocialMediaService).getRecentPosts(
            {
      ...
      scriptingService.registerFunction("getCurrentSocialPost", {
        ...
        execute(this: ScriptingThis): string {
          const post = this.agent.requireServiceByType(SocialMediaService).getCurrentPost(this.agent);
          return post ? JSON.stringify(post) : "No social media post selected";
        },
      });
    });
