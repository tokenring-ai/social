...
async function execute({ title, content, replyToPostId, metadata }: z.output<typeof inputSchema>, agent: Agent): Promise<TokenRingToolResult> {
...
const inputSchema = z.object({
  title: z.string().exactOptional().describe("Optional title used by providers that support titled posts"),
  content: z.string().describe("The text content of the social media post"),
  replyToPostId: z.string().exactOptional().describe("Optional post ID to reply to"),
  metadata: z.record(z.string(), z.unknown()).exactOptional().describe("Provider-specific metadata such as subreddit or link URL"),
});
